...
class WebviewController extends ValueNotifier<WebviewValue> {
  ...
  /// Returns a request-style cookie header for the provided [url].
  ///
  /// The header includes cookies managed by WebView2 for that URL, including
  /// `HttpOnly` cookies that are not visible through `document.cookie`.
  Future<String?> getCookieHeader(String url) async {
    if (_isDisposed) {
      return null;
    }
    assert(value.isInitialized);
    return _methodChannel.invokeMethod<String>('getCookieHeader', url);
  }
