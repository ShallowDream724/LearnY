...
constexpr auto kMethodGetCookieHeader = "getCookieHeader";
...
void WebviewBridge::HandleMethodCall(
  ...
  if (method_name.compare(kMethodGetCookieHeader) == 0) {
    if (const auto url = std::get_if<std::string>(method_call.arguments())) {
      std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>>
          shared_result = std::move(result);
      webview_->GetCookieHeader(
          *url, [shared_result](bool success, const std::string& header) {
            if (success) {
              shared_result->Success(header);
            } else {
              shared_result->Error(kMethodFailed, "Reading cookies failed.");
            }
          });
      return;
    }
