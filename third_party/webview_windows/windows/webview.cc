...
void Webview::GetCookieHeader(const std::string& url,
                              CookieHeaderCallback callback) {
  if (!IsValid()) {
    callback(false, std::string());
    return;
  }

  wil::com_ptr<ICoreWebView2_2> webview2 = webview_.query<ICoreWebView2_2>();
  if (!webview2) {
    callback(false, std::string());
    return;
  }
  ...
  const auto wide_url = util::Utf16FromUtf8(url);
  const auto hr = cookie_manager->GetCookies(
      ...
      Callback<ICoreWebView2GetCookiesCompletedHandler>(
          [callback = std::move(callback)](HRESULT result,
              ...
              -> HRESULT {
            if (FAILED(result) || cookie_list == nullptr) {
              callback(false, std::string());
              return S_OK;
            }
            ...
            for (UINT index = 0; index < count; ++index) {
              ...
              if (FAILED(cookie_list->GetValueAtIndex(index, cookie.put())) ||
                  !cookie) {
                continue;
              }
              ...
              header += util::Utf8FromUtf16(name.get());
              header += "=";
              header += util::Utf8FromUtf16(value.get());
            }
