...
class _PdfViewerState extends State<PdfViewer>
  ...
  static const _selectionHandleAutoScrollEdgeExtent = 56.0;
  static const _selectionHandleAutoScrollMaxStep = 28.0;
  ...
  Offset? _selectionHandleDragGlobalPosition;
  ...
  void _onSelectionHandlePanStart(_TextSelectionPart handle, DragStartDetails details) {
    ...
    _isSelectingAllText = false;
    _selectionHandleDragGlobalPosition = details.globalPosition;
    ...
    _updateTextSelection();
    _scheduleSelectionHandleAutoScroll();
    _requestFocus();
  }
  ...
  void _scheduleSelectionHandleAutoScroll() {
    if (_selectionHandleDragGlobalPosition == null ||
        (_selPartMoving != _TextSelectionPart.a && _selPartMoving != _TextSelectionPart.b)) {
      _stopSelectionHandleAutoScroll();
      return;
    }

    if (_computeSelectionHandleAutoScrollStep() == Offset.zero) {
      _stopSelectionHandleAutoScroll();
      return;
    }
    ...
    _selectionHandleAutoScrollTimer ??= Timer.periodic(
      ...
      (_) => _performSelectionHandleAutoScroll(),
    );
  }
  ...
  void _stopSelectionHandleAutoScroll() {
    ...
    _selectionHandleDragGlobalPosition = null;
  }

  void _performSelectionHandleAutoScroll() {
    if (!mounted || _viewSize == null) {
      _stopSelectionHandleAutoScroll();
      return;
    }

    final step = _computeSelectionHandleAutoScrollStep();
    if (step == Offset.zero) {
      _stopSelectionHandleAutoScroll();
      return;
    }

    final globalPosition = _selectionHandleDragGlobalPosition;
    if (globalPosition == null) {
      _stopSelectionHandleAutoScroll();
      return;
    }

    final previousMatrix = _txController.value.clone();
    final previousVisibleRect = _visibleRect;
    final nextCenter = _txController.value
        .calcPosition(_viewSize!)
        .translate(step.dx / _currentZoom, step.dy / _currentZoom);
    final nextMatrix = _makeMatrixInSafeRange(
      _calcMatrixFor(nextCenter, zoom: _currentZoom, viewSize: _viewSize!),
      forceClamp: true,
    );
    final nextVisibleRect = nextMatrix.calcVisibleRect(_viewSize!);

    if (nextVisibleRect == previousVisibleRect) {
      _stopSelectionHandleAutoScroll();
      return;
    }

    _txController.value = nextMatrix;
    if (_textSelectAnchor != null) {
      final translationDelta = Offset(nextMatrix.x - previousMatrix.x, nextMatrix.y - previousMatrix.y);
      // Drag math already subtracts the current matrix translation, so when
      // auto-scroll moves the viewport we need to carry the anchor along once.
      _textSelectAnchor = _textSelectAnchor! + translationDelta;
    }
    _updateSelectionHandlesPan(_globalToDocument(globalPosition));
  }

  Offset _computeSelectionHandleAutoScrollStep() {
    final globalPosition = _selectionHandleDragGlobalPosition;
    final viewSize = _viewSize;
    if (globalPosition == null || viewSize == null) {
      return Offset.zero;
    }
    ...
    final thresholdX = min(_selectionHandleAutoScrollEdgeExtent, viewSize.width / 2);
    final thresholdY = min(_selectionHandleAutoScrollEdgeExtent, viewSize.height / 2);

    return Offset(
      _computeSelectionHandleAutoScrollAxisDelta(localPosition.dx, viewSize.width, thresholdX),
      _computeSelectionHandleAutoScrollAxisDelta(localPosition.dy, viewSize.height, thresholdY),
    );
  }

  double _computeSelectionHandleAutoScrollAxisDelta(double position, double extent, double threshold) {
    if (threshold <= 0 || extent <= 0) {
      return 0;
    }

    if (position < threshold) {
      final intensity = ((threshold - position) / threshold).clamp(0.0, 1.0);
      return -_selectionHandleAutoScrollMaxStep * Curves.easeOut.transform(intensity);
    }

    final trailingEdge = extent - threshold;
    if (position > trailingEdge) {
      final intensity = ((position - trailingEdge) / threshold).clamp(0.0, 1.0);
      return _selectionHandleAutoScrollMaxStep * Curves.easeOut.transform(intensity);
    }
